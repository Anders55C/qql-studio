...
use super::layouts::{
    FormationDims, OrbitalInfo, ShadowsInfo, StartPointGroups, StructureInfo,
};
...
impl GroupedFlowLines {
    pub fn build(
        ...
        Self::build_with_info(flow_field, ignore_flow_field, start_point_groups, rng).0
    }

    /// Like [`build`] but also returns the `curve_length` chosen for this
    /// piece (`500`, `650`, or `850`). The same value is used for every flow
    /// line in the piece.
    pub fn build_with_info(
        ...
        (GroupedFlowLines(groups), curve_length)
    }
...
impl Points {
    ...
    pub fn build(
        ...
        .expect("no deadline => never aborts")
    }

    /// Like [`build`], but if `deadline` is `Some`, periodically checks the
    /// clock between flow lines and returns `None` if the deadline has passed.
    /// Used to bound worst-case time on dense pieces.
    #[allow(clippy::too_many_arguments)]
    pub fn build_with_deadline(
        ...
        Some((Points(all_points), GroupSizes(group_sizes)))
    }

    /// Returns `true` if the build was aborted because the deadline expired.
    #[allow(clippy::too_many_arguments)]
    fn build_group(
        dest: &mut Vec<Point>,
        color_db: &ColorDb,
...
pub struct RenderData {
    ...
    pub radius_buckets: RadiusBuckets,
    pub quadrant_counts: QuadrantCounts,
    pub background_color: ColorKey,
    pub formation_dims: Option<FormationDims>,
    pub orbital_info: Option<OrbitalInfo>,
    pub shadows_info: Option<ShadowsInfo>,
...
pub fn draw<F: FnMut(Frame)>(
    ...
    let (start_points, structure_info) =
        StartPointGroups::build_with_info(traits.structure, &mut rng);
    let formation_dims_summary: Option<FormationDims> = structure_info.formation;
    let orbital_info_summary: Option<OrbitalInfo> = structure_info.orbital;
    let shadows_info_summary: Option<ShadowsInfo> = structure_info.shadows;

    let (grouped_flow_lines, curve_length_summary) =
        GroupedFlowLines::build_with_info(flow_field, ignore_flow_field, start_points, &mut rng);
    ...
    let radius_buckets_summary = radius_buckets(&points.0);
    let quadrant_counts_summary = quadrant_counts(&points.0);
    ...
    let background_color = color_scheme.background;
    let splatter_odds_summary = color_scheme.splatter_odds;
    RenderData {
        canvas: dt,
        num_points,
        colors_used,
        ring_counts_used,
        radius_buckets: radius_buckets_summary,
        quadrant_counts: quadrant_counts_summary,
        background_color,
        formation_dims: formation_dims_summary,
        orbital_info: orbital_info_summary,
        shadows_info: shadows_info_summary,
        curve_length: curve_length_summary,
        splatter_odds: splatter_odds_summary,
    }
}
...
pub struct LayoutSummary {
    ...
    pub quadrant_counts: QuadrantCounts,
    pub background_color: ColorKey,
    /// For Formation pieces only: the `(H, V)` grid the algorithm chose.
    pub formation_dims: Option<FormationDims>,
    /// For Orbital pieces only: ring band count, splits range, center location.
    pub orbital_info: Option<OrbitalInfo>,
    /// For Shadows pieces only: circle count, fill style, etc.
    pub shadows_info: Option<ShadowsInfo>,
    /// Curve length picked for the flow lines (500 / 650 / 850). Same value
    /// is used for every flow line in the piece.
    pub curve_length: usize,
    /// Per-point probability of spawning splatter satellites. `0.0` means the
    /// piece has no splatter at all. Possible non-zero values (from the
    /// algorithm's weighted choices): 0.001, 0.002, 0.005, 0.01, 0.03, 0.08,
    /// 0.5 — gated by `color_variety`.
    pub splatter_odds: f64,
}

/// Number of drawn points (those with positive radius) whose centres fall in
/// each visible quadrant of the canvas. Useful for spotting density gradients
/// — e.g. "dense in the top-left, sparse in the bottom-right".
...
fn quadrant_counts(points: &[Point]) -> QuadrantCounts {
    let mid_x = VIRTUAL_W * 0.5;
    let mid_y = VIRTUAL_H * 0.5;
    ...
    for pt in points {
        ...
        let (x, y) = pt.position;
        // Only count points whose centres are on the visible canvas.
        if x < 0.0 || x > VIRTUAL_W || y < 0.0 || y > VIRTUAL_H {
            continue;
        }
        let left = x < mid_x;
        let top = y < mid_y;
        match (top, left) {
            (true, true) => q.top_left += 1,
            (true, false) => q.top_right += 1,
            (false, true) => q.bottom_left += 1,
            (false, false) => q.bottom_right += 1,
        }
    }
    q
}

/// Histogram of drawn-point radii, bucketed by absolute size on the virtual canvas
/// (which is 2000px wide, scaled up at paint time).
///
/// Thresholds (in virtual canvas units):
/// * `small`: 0 < r ≤ 24px (≤1.2% canvas)
/// * `medium`: 24 < r ≤ 60px (1.2–3% canvas)
/// * `large`: r > 60px (>3% canvas)
///
/// Points with non-positive radii (which are placed by the algorithm but not
/// drawn) are not counted.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct RadiusBuckets {
    pub small: usize,
    pub medium: usize,
    pub large: usize,
}

/// Boundary between small and medium points on the virtual canvas.
pub const RADIUS_BUCKET_SMALL_MAX: f64 = VIRTUAL_W * 0.012;
/// Boundary between medium and large points on the virtual canvas.
pub const RADIUS_BUCKET_MEDIUM_MAX: f64 = VIRTUAL_W * 0.030;

fn radius_buckets(points: &[Point]) -> RadiusBuckets {
    let mut b = RadiusBuckets::default();
    for pt in points {
        if pt.scale <= 0.0 {
            continue;
        }
        if pt.scale <= RADIUS_BUCKET_SMALL_MAX {
            b.small += 1;
        } else if pt.scale <= RADIUS_BUCKET_MEDIUM_MAX {
            b.medium += 1;
        } else {
            b.large += 1;
        }
...
/// Run only the layout phase of the QQL algorithm and return summary statistics
/// (traits, point count, colors used, ring-count histogram). Skips rasterization,
/// so this is much cheaper than [`draw`] and suitable for bulk filtering.
pub fn layout(seed: &[u8; 32], color_db: &ColorDb, config: &Config) -> LayoutSummary {
    layout_with_deadline(seed, color_db, config, None)
        .expect("no deadline => never aborts")
}

/// Like [`layout`] but bails out and returns `None` if `deadline` has passed
/// before the point-placement phase finishes. Use this to bound worst-case
/// wall time on dense pieces during bulk filtering.
pub fn layout_with_deadline(
    seed: &[u8; 32],
    color_db: &ColorDb,
    config: &Config,
    deadline: Option<Instant>,
) -> Option<LayoutSummary> {
    let traits = Traits::from_seed(seed);
    let mut rng = Rng::from_seed(&seed[..]);
    ...
    let (start_points, structure_info) =
        StartPointGroups::build_with_info(traits.structure, &mut rng);
    let formation_dims = structure_info.formation;
    let orbital_info = structure_info.orbital;
    let shadows_info = structure_info.shadows;

    let (grouped_flow_lines, curve_length) =
        GroupedFlowLines::build_with_info(flow_field, ignore_flow_field, start_points, &mut rng);
    let mut sectors: Sectors = build_sectors(config);
    let mut colors_used = ColorsUsed::new();
    let (points, _group_sizes) = Points::build_with_deadline(
        &traits,
        color_db,
        grouped_flow_lines,
        &color_scheme,
    ...
    let radius_buckets = radius_buckets(&points.0);
    let quadrant_counts = quadrant_counts(&points.0);
    let background_color = color_scheme.background;
    let splatter_odds = color_scheme.splatter_odds;
